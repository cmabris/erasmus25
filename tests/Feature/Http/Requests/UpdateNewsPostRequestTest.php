...
describe('UpdateNewsPostRequest - Authorization', function () {
    it('authorizes user with edit permission', function () {
        ...
        $request = UpdateNewsPostRequest::create(
            "/admin/noticias/{$newsPost->id}",
            'PUT',
            [
                'academic_year_id' => $academicYear->id,
                'title' => 'Updated News',
        ...
        $request->setRouteResolver(function () use ($newsPost) {
            $route = new \Illuminate\Routing\Route(['PUT'], '/admin/noticias/{news_post}', []);
            $route->bind(new \Illuminate\Http\Request);
            $route->setParameter('news_post', $newsPost);
    ...
    it('returns false when route parameter is not NewsPost instance', function () {
        $user = User::factory()->create();
        $user->assignRole(Roles::ADMIN);
        $this->actingAs($user);
        ...
        $request->setRouteResolver(function () {
            $route = new \Illuminate\Routing\Route(['PUT'], '/admin/noticias/{news_post}', []);
            $route->bind(new \Illuminate\Http\Request);
            $route->setParameter('news_post', 'not-a-news-post'); // No es instancia de NewsPost
    ...
    it('returns false when route parameter is null', function () {
        ...
        $request->setRouteResolver(function () {
            $route = new \Illuminate\Routing\Route(['PUT'], '/admin/noticias/{news_post}', []);
            $route->bind(new \Illuminate\Http\Request);
            $route->setParameter('news_post', null);
...
describe('UpdateNewsPostRequest - Validation Rules', function () {
    it('returns rules with ignored slug when route parameter is NewsPost instance', function () {
        $user = User::factory()->create();
        $user->assignRole(Roles::ADMIN);
        $this->actingAs($user);
    ...
    it('handles route parameter when it is not NewsPost instance', function () {
        $user = User::factory()->create();
        $user->assignRole(Roles::ADMIN);
        $this->actingAs($user);
        ...
        $request->setRouteResolver(function () {
            $route = new \Illuminate\Routing\Route(['PUT'], '/admin/noticias/{news_post}', []);
            $route->bind(new \Illuminate\Http\Request);
            $route->setParameter('news_post', 999); // ID numérico, no instancia
    ...
    it('validates slug uniqueness ignoring current news post', function () {
        $user = User::factory()->create();
        $user->assignRole(Roles::ADMIN);
        $this->actingAs($user);
        ...
        $request = UpdateNewsPostRequest::create(
            "/admin/noticias/{$newsPost2->id}",
            'PUT',
            []
        );
        $request->setRouteResolver(function () use ($newsPost2) {
            $route = new \Illuminate\Routing\Route(['PUT'], '/admin/noticias/{news_post}', []);
            $route->bind(new \Illuminate\Http\Request);
            $route->setParameter('news_post', $newsPost2);
        ...
        // Debe permitir usar el mismo slug del newsPost2
        $validator = Validator::make([
            'academic_year_id' => $academicYear->id,
            'title' => 'Updated News',
            'content' => 'Updated content',
            'slug' => 'another-slug', // Mismo slug del newsPost2
        ], $rules);

        expect($validator->fails())->toBeFalse();

        // Pero no debe permitir usar el slug de otro newsPost
        $validator2 = Validator::make([
            'academic_year_id' => $academicYear->id,
            'title' => 'Updated News',
            'content' => 'Updated content',
            'slug' => 'existing-slug', // Slug de newsPost1
        ], $rules);
...
describe('UpdateNewsPostRequest - Custom Messages', function () {
