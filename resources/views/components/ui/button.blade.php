...
@php
    ...
    $variantClasses = match($buttonVariant) {
...
>
    ...
    @elseif($icon)
        <flux:icon :name="$icon" :class="$iconSizeClasses" variant="outline" />
    @endif
    ...
    @if($iconTrailing && !$loading)
        <flux:icon :name="$iconTrailing" :class="$iconSizeClasses" variant="outline" />
    @endif
</{{ $tag }}>
