<div>
    ...
    <div class="mb-6 animate-fade-in">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-zinc-900 dark:text-white">
                    {{ __('Editar Resolución') }}
                </h1>
                <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
                    {{ __('Modifica la información de la resolución: :title', ['title' => $this->resolution->title ?? __('Resolución')]) }}
                </p>
            </div>
            <flux:button 
        ...
        <x-ui.breadcrumbs 
            ...
            :items="[
                ...
                ['label' => $this->resolution->title ?? __('Resolución'), 'href' => isset($this->resolution->id) ? route('admin.calls.resolutions.show', [$call, $this->resolution]) : route('admin.calls.resolutions.index', $call), 'icon' => 'document-check'],
                            ...
                            @if($this->existingPdf && !$removeExistingPdf)
                                ...
                                <flux:field>
                                    ...
                                    <div class="flex items-center gap-4 rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-800">
                                        <flux:icon name="document-text" class="[:where(&)]:size-8 text-red-600 dark:text-red-400" variant="outline" />
                                        <div class="flex-1">
                                    ...
                                    <flux:description>
                                        {{ __('Puedes eliminar este PDF y subir uno nuevo, o mantenerlo.') }}
                                    </flux:description>
                                </flux:field>
                            @else
                                {{-- Upload New PDF --}}
                    ...
                    <x-ui.card>
                        <div class="space-y-4">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <flux:button 
                                    type="button"
                                    href="{{ route('admin.calls.resolutions.show', [$call, $this->resolution]) }}" 
                                    variant="ghost"
                                    wire:navigate
                                    class="w-full"
                                >
    ...
    {{-- Toast Notifications --}}
    <x-ui.toast event="resolution-updated" variant="success" />
</div>
